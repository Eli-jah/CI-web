...
$IP = get_ip();
    ...
    $region_data = get_region('shm_city','id,yid,title');
    $city_parent_id = get_Simplify('shm_city','yid',array('title'=>$region));
    if (!empty($city_parent_id))
    {
    	$city_data = get_Simplify('shm_city','id,yid,title',array('parent_id'=>$city_parent_id->yid),'1');
    }
